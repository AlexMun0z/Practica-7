...
const char* ssid = "chamba";
const char* password = "abc123ir";
...
#endif
```
### Descripció general del funcionament
- Configuració del Wi-Fi: La placa es connecta a la xarxa Wi-Fi especificada.
- Inicialització del servidor web: Es configura un servidor web a la porta 80.
- Interacció amb l'usuari: L'usuari pot interactuar amb la pàgina web per veure l'estat actual, canviar l'URL de streaming, ajustar el volum o aturar la reproducció.
- Reproducció d'àudio: Es gestiona la reproducció d'àudio mitjançant les llibreries d'àudio, que permeten el streaming de fitxers MP3 o AAC.
- Emmagatzematge en EEPROM: La configuració es guarda a l'EEPROM per mantenir l'estat entre reinicis.

  
### Detalls tècnics
- Preallocació de memòria: Es preal·loca memòria per a la memòria intermèdia i el còdec d'àudio per evitar problemes de fragmentació de memòria.
- Callbacks de metadades i estat: S'utilitzen callbacks per gestionar les metadades del flux d'àudio i l'estat del sistema.
- Gestió de l'URL de streaming: Quan es canvia l'URL, es reinicia la reproducció amb el nou URL especificat.
  
Aquest codi proporciona una base per construir un receptor de ràdio en streaming utilitzant una placa ESP8266 o ESP32, permetent el control a través d'una interfície web simple.
